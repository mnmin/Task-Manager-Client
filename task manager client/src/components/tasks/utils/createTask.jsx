...
export const FormCreateTaskDialog = ({ openDialog, setOpenDialog }) => {
  ...
  const createNewTask = async (setResponse, taskName, taskDescription, linksUrl, topics, status, priority) => {
    try {
      const { data } = await client.post('/task/', { taskName, taskDescription, linksUrl, topics, status, priority, user: { id:  user.id } });
      setResponse(data);
      return;
    } catch (error) {
  ...
  const handleCreateTask = async () => {
    const res = await createNewTask(setTaskResponse, editTaskName, editTaskDescription, editTaskLinksUrl, taskTopics, editTaskStatus, editTaskPriority);
    if (res?.status === "fail") {
      ...
      <Dialog open={openDialog} scroll="body">
        ...
        <li>
          {newTopic ? (
            ...
            <InputLabel id="create-task-status-select-label">Task Status</InputLabel>
              <Select
